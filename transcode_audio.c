...
unsigned long out_sampling_rate;
...
static int convert_samples(const uint8_t **input_data,
                           ...
                           SwrContext *resample_context,
                           unsigned long *dest_n_of_sample)
{
    int error;

    unsigned long dst_nb_samples;
                                         ...
                                         int *finished)
{
    ...
    unsigned long n_of_conv_samples;
    ...
    if (data_present) {
        ...
        if (convert_samples((const uint8_t**)input_frame->extended_data, converted_input_samples,
                            input_frame->nb_samples, resampler_context, &n_of_conv_samples))
            goto cleanup;
...
cleanup:
    if (converted_input_samples) {
        int stocazzo = &converted_input_samples[0];
        stocazzo++;
        stocazzo--;
        av_freep(&converted_input_samples[0]);
        free(converted_input_samples);
                              ...
                              int *data_present)
{
    ...
    if (*data_present) {
        ...
        for(c1=0; c1<output_packet.size; c1++){

            out_buffer[*out_buffer_len] = output_packet.data[c1];
                  ...
                  unsigned long *out_buff_len)
{
    ...
    out_sampling_rate = out_sampl_rate;

    *out_buffer_len = 0;

    av_register_all();

    if (open_input_file(in_file_name, &input_format_context,
                        &input_codec_context))
        goto cleanup;


    if (open_output_file(in_file_name_wav, input_codec_context,
                  ...
                  unsigned long *out_buff_len)
{
    ...
    *out_buffer_len = 0;
    ...
    if (open_output_file("temp_output.wav", input_codec_context,
                         &output_format_context, &output_codec_context))
        goto cleanup;
